...
class DetailedStatsCallback(BaseCallback):
    def __init__(self, log_dir: str = "./logs", save_freq: int = 10000, verbose=0):
        ...
        self.csv_path = os.path.join(log_dir, "summary_stats.csv")
    ...
    def _on_step(self) -> bool:
        ...
        self.step_actions.extend(np.linalg.norm(actions, axis=-1).tolist())
        self.step_obs_means.extend(np.mean(obs, axis=-1).tolist())

        self._recent_rewards.extend(rewards)
        ...
        std_reward = np.std(self._recent_rewards) if len(self._recent_rewards) > 0 else 0
        max_reward = np.max(self._recent_rewards) if len(self._recent_rewards) > 0 else 0
        min_reward = np.min(self._recent_rewards) if len(self._recent_rewards) > 0 else 0
        mean_action_mag = np.mean(self.step_actions[-len(rewards):]) if len(rewards) > 0 else 0
        mean_obs_mean = np.mean(self.step_obs_means[-len(rewards):]) if len(rewards) > 0 else 0
        num_dones = sum(dones)


        self.logger.record("stats/mean_step_reward", mean_reward)
        self.logger.record("stats/std_step_reward", std_reward)
        self.logger.record("stats/max_step_reward", max_reward)
        self.logger.record("stats/min_step_reward", min_reward)
        self.logger.record("stats/action_magnitude_mean", mean_action_mag)
        self.logger.record("stats/obs_mean", mean_obs_mean)
        self.logger.record("stats/num_dones", num_dones)
        self.logger.record("stats/episodes_completed", episodes_completed)

        if len(self.episode_rewards) > 0:
            self.logger.record("stats/mean_episode_reward", np.mean(self.episode_rewards))
            self.logger.record("stats/std_episode_reward", np.std(self.episode_rewards))
            self.logger.record("stats/mean_episode_length", np.mean(self.episode_lengths))

        if self.num_timesteps % self.save_freq == 0:
            model_path = os.path.join(self.log_dir, f"model_step_{self.num_timesteps}.zip")
            self.model.save(model_path)
            if self.verbose > 0:
                print(f"Saved model at step {self.num_timesteps} to {model_path}")

            # Save numpy arrays
            np.save(os.path.join(self.log_dir, "step_rewards.npy"), np.array(self.step_rewards))
            np.save(os.path.join(self.log_dir, "step_actions.npy"), np.array(self.step_actions))
            np.save(os.path.join(self.log_dir, "step_obs_means.npy"), np.array(self.step_obs_means))
            np.save(os.path.join(self.log_dir, "episode_rewards.npy"), np.array(self.episode_rewards))
            np.save(os.path.join(self.log_dir, "episode_lengths.npy"), np.array(self.episode_lengths))

            # Append summary stats to CSV
    ...
    def _on_training_end(self) -> None:
        ...
        if self.verbose:
            ...
            print(f"Logged {len(self.step_rewards)} step rewards")
            print(f"Logged {len(self.episode_rewards)} completed episodes")
            print(f"Summary CSV saved to {self.csv_path}")
